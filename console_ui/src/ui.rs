...
fn get_title<'a>(app: &App) -> Paragraph<'a> {
  let text = vec![
    match app.layer {
    ...
    match app.current_screen {
      ...
      CurrentScreen::AccumulatedWeights => {
        Span::styled(format!("Accumulated for {}, {}", app.neuron_x, app.neuron_y), Style::default())
      },
      CurrentScreen::DistanceWeights => {
        Span::styled(format!("Distances for {}, {}", app.neuron_x, app.neuron_y), Style::default())
      },
    }
    .to_owned(),
  ];

  return Paragraph::new(Line::from(text))
