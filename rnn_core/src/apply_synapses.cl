...
__kernel void apply_synapses(
    ...
    if (refract_intervals_to[row] > 0) {
        ...
        ulong index_from = offsets[row] + row % field_size;
        ...
        if (offsets[row] < layer_size) {
            for (uint col = 0; col < field_size; ++col) {
                ...
                unsigned int distance_index = row * layer_size + neuron_from_index;
                ...
                if (neurons_from[neuron_from_index] > 0) {
                    ...
                    if (weight_to > 0.0001 || weight_to < -0.0001) {
                        sum += get_weight_coefficient(gamma_inc, gamma_dec, weight_to, g_0) * distance_weights[distance_index];
                    }
                }
            }
        }
    ...
    if (offsets[row] < layer_size) {
        for (uint col = 0; col < field_size; ++col) {
            ...
            if (neurons_from[neuron_from_index] > 0) {
                if (distance_weights[distance_index] < 0.001) {
                    continue;
                }
