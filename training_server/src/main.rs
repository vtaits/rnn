...
async fn send_data_to_receiver(receiver: &str, data: Arc<Vec<u8>>, semaphore: Arc<Semaphore>) -> tokio::io::Result<()> {
...
async fn update_receivers(
    data: web::Data<AppState>,
) -> impl Responder {
    let data_layer = data.data_layer.lock().unwrap();

    let compressed_data = data_layer.get_network().read().unwrap().get_gzip_dump().unwrap();
    ...
    for receiver in &data.receivers {
        ...
        let task = tokio::spawn(async move {
            timeout(Duration::from_secs(5), send_data_to_receiver(&receiver, data_for_send, semaphore)).await
        });
...
async fn main() -> std::io::Result<()> {
    ...
    // let receivers = receivers_str.split(',').collect::<Vec<&str>>();
    let receivers = receivers_str.split(',').map(|part| String::from(part)).collect();
